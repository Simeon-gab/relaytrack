...
| 4 | Dispatcher live map | 🟨 Code complete (2026-08-16) | Build + all 24 tests pass (16 exception fire/clear cases; realtime org-isolation proven on the wire with positive control). Two-rider sims drove the real authenticated ingest pipeline end-to-end repeatedly (200s, zero discards, last_position current). Browser realtime verified: on a clean load the app's markers stay fresh minutes after SSR — only live events can do that; Node probes confirm the org_id filter and the StrictMode subscribe/remove/resubscribe pattern both deliver. Added subscribe-status error logging (dead channels were previously silent). **Eyeballs-on-screen check pending** — display was asleep/occluded all session so Chrome suspended rendering (rAF=0) and kept freezing the tab; with the screen on, open /dispatch while `npm run simulate` runs (scenarios: `--scenario=stall\|offline\|offroute`) |
